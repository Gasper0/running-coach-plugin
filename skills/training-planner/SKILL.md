...
- **If `show_widget` tool is available** (typically Claude.ai web) → use Path A (interactive widget)
- **If `show_widget` is unavailable** (typically Claude Code CLI, API, terminal) → use Path B (structured text protocol)
...
- Objectif : [distance] le [date], chrono cible [time]
- Profil : [experience], [weekly_volume]km/semaine, PB récent [pb]
- Dispo : [N] séances/semaine, jours [days], max [duration] par séance
- Physio : [age] ans, [sex], FC max [hr_max] bpm
- Exports : [exports]

C'est correct ? On peut passer aux zones et au plan.
```
...
The output is a zone table:

| Zone | Pace range | HR range | % FC max |
|---|---|---|---|
| Z1 — Recovery | ... | ... | <65% |
| Z2 — Endurance | ... | ... | 65-75% |
| Z3 — Tempo | ... | ... | 75-85% |
...
- `date`, `week`, `phase`, `session_type`, `code`, `title`
- `duration_min`, `distance_km`, `zones`, `pace_target`, `hr_zone`
- `description` (warmup → main → cooldown structure)
- `notes` (coaching cues)

For Hyrox plans, include a "Run Brick" session type once a week (running + 1-2 stations). Do NOT generate detailed station protocols here — the user uses `workout-builder` for that level of detail. Run Brick sessions are placeholders with general guidance ("4 rounds, 1km run + 1 station each").
...
#### If `show_widget` available
Use a `show_widget` table format with color coding from `../_shared/session-schema.md`.

#### Fallback if `show_widget` unavailable
Use a structured markdown summary in this format:

```
## Plan summary — [Distance] [Date] — Goal [Time]

### Phase breakdown
| Phase | Weeks | Focus | Peak volume |
|---|---|---|---|
| Base | W1-W3 | Aerobic base | 35 km |
| Build | W4-W7 | Threshold | 48 km |
| Peak | W8-W10 | VMA + race pace | 52 km |
| Taper | W11-W12 | Recovery + sharpening | 28 km |

### Week-by-week
| Week | Phase | Tue | Thu | Sun | Total km |
|---|---|---|---|---|---|
| W1 | Base | 🟢 EF 45min | 🟠 Tempo 30min | 🔵 SL 12km | 35 |
| W2 | ... | ... | ... | ... | ... |

### Total volume: 460 km over 12 weeks
```

Wait for explicit user validation ("c'est bon, on pousse") in both paths. Do not skip this step.

### Stage 6 — Exports (CRITICAL — workflow differs by target)
...
- The agenda shows the whole roadmap; the user wants to see the next 12 weeks at a glance
...
**Garmin Connect — current week only, refresh weekly:**
- The watch shouldn't be cluttered with 12 weeks of workouts
...
- This is a different interaction than the initial plan creation — it can be a quick command like "pousse la semaine 3 sur Garmin"

For Garmin pushes, use the rules from `workout-builder/references/garmin-workout-rules.md` (pace as integer sec/km, flat repeat blocks, mandatory open rest between warmup and main, never call schedule_workout twice).

After initial creation, end the session with a reminder: "Le plan complet est dans ton agenda. Pour Garmin, je n'ai poussé que la semaine 1. Dis-moi 'pousse la semaine 2 sur Garmin' au début de la semaine prochaine."

### Optional exports (on demand only)

Excel/CSV, Word/PDF — only if the user explicitly asks. Don't propose proactively. Implementations live in `references/` if needed:
- `references/excel-export.md`
- `references/docx-export.md`

These formats are useful for sharing/archiving but not for daily use.
...
**Never place a quality session within 48h of another.** If the user's available days don't allow this, flag it and propose a downgrade of one session to easy.

### Cross-training interference

If the user does strength training (especially leg work), do NOT schedule it the day before VMA. The legacy rule: strength legs goes on Saturday (between Friday quality and Sunday long run) to enable a back-to-back stimulus useful for Hyrox prep. Avoid Monday strength legs (kills Tuesday VMA).

### Hyrox + running combo

If the user is preparing both a Hyrox event and a running race in parallel (e.g. 10km + Hyrox Solo), prioritize the proximity-priority race. Defer the secondary objective by reducing its quality block intensity.
...
### MCP tools used
- **`google-calendar:list-calendars`** + **`google-calendar:create-event`** — for calendar exports
- **`garmin:create_workout`** + **`garmin:schedule_workout`** + **`garmin:list_workouts`** — for Garmin pushes
...
### File outputs (depending on user choices)
- Calendar events (via MCP, no local files)
- `.ics` file in `/mnt/user-data/outputs/` if user prefers that over MCP
- Optional: `.xlsx`, `.docx`, `.pdf` if user requests
...
- [`references/garmin-export.md`](references/garmin-export.md) — Garmin-specific export details (coordinates with `workout-builder/references/garmin-workout-rules.md`)
...
- **For the next Garmin push** → user activates this skill briefly with "pousse la semaine X sur Garmin"
